...
function LimitModal(props) {
  var resetDate = props.resetDate;
  var resetStr = resetDate ? new Date(resetDate).toLocaleDateString("en-US", { day:"numeric", month:"short", year:"numeric" }) : "30 days from your first use";
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      <div style={{ background:BRAND.white, maxWidth:420, width:"100%", padding:32, position:"relative" }}>
        ...
        <div style={{ fontSize:14, color:BRAND.muted, marginBottom:28, lineHeight:1.6 }}>You have two options — upgrade for unlimited access, or wait for your free limit to reset.</div>
        ...
        <div style={{ border:`2px solid ${BRAND.black}`, padding:20, marginBottom:12, position:"relative" }}>
          ...
          <div style={{ fontSize:13, color:BRAND.muted, marginBottom:16 }}>Unlimited AI generations, all platforms, all sections — $19/month or $15/month annually.</div>
          <button style={{ background:BRAND.rainbow, color:BRAND.white, border:"none", padding:"12px 24px", fontWeight:700, fontSize:14, cursor:"pointer", width:"100%" }} onClick={props.onUpgrade}>
            Upgrade Now →
          </button>
        </div>
        ...
        <div style={{ border:`1px solid ${BRAND.border}`, padding:20 }}>
          ...
          <div style={{ fontSize:13, color:BRAND.muted, marginBottom:16 }}>Your 15 free generations reset on <strong>{resetStr}</strong>. Come back then to continue for free.</div>
          <button style={{ background:BRAND.bg, color:BRAND.black, border:`1.5px solid ${BRAND.border}`, padding:"12px 24px", fontWeight:700, fontSize:14, cursor:"pointer", width:"100%" }} onClick={props.onClose}>
            I will wait
          </button>
        </div>
      </div>
...
export default function App() {
  ...
  async function loadUserData(userId) {
    try {
      ...
      var d = json.data;
      if (!d || !d.pack) {
        // New user — clear state, go to onboarding
        ...
        setGens(0); setIsPro(false); setNav(0); setObStep(0);
  ...
  async function handleEmailAuth() {
    ...
    try {
      if (authMode === "signup") {
        ...
        await loadUserData(r2.data.user.id);
      }
  ...
  function canGen() {
    ...
    if (gensResetDate && new Date() > new Date(gensResetDate)) {
      setGens(0); setGensResetDate(null);
      saveUserData({ gens:0, gensResetDate:null });
      return true;
    }
  ...
  function useGen(updates) {
    ...
    // Set reset date on first generation
    if (n === 1 && !resetDate) {
      var d = new Date();
      d.setDate(d.getDate() + RESET_DAYS);
      resetDate = d.toISOString();
      setGensResetDate(resetDate);
    }
    setGens(n);
  ...
  async function genMonthPlan(idx) {
    ...
    try {
      ...
      if (data) {
        ...
        setMonthPlans(u); saveUserData({ monthPlans:u });
      }
    } catch(e) {}
  ...
  async function genShoot(idea) {
    ...
    try {
      ...
      setShoots(u); saveUserData({ shoots:u });
    } catch(e) {
  ...
  async function genHooks(idea) {
    ...
    try {
      ...
      if (data) {
        ...
        setHooks(u); saveUserData({ hooks:u });
      }
    } catch(e) {}
    setLoading("");
  }
  ...
  async function sendChat() {
    ...
    try {
      ...
      var fm = nm.concat([{ role:"assistant", content:txt }]);
      setChat(fm); saveUserData({ chat:fm });
    } catch(e) {}
    setLoading("");
        ...
        <div style={{ maxWidth:860, margin:"0 auto", padding:"60px 24px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:16 }}>
            {[
              ...
              { emoji:"💡", title:"Endless Ideas", desc:"Generate platform-specific content ideas instantly. Never run out." },
              { emoji:"🎬", title:"Shoot Plans", desc:"Full scripts, shot lists and repurposing ideas for every piece of content." },
              { emoji:"🪝", title:"Hooks and Captions", desc:"5 hook types and platform-native captions for every post." },
              { emoji:"📅", title:"Content Scheduler", desc:"Drag and drop your content into a 4-week calendar." },
              { emoji:"🤖", title:"AI Advisor", desc:"Ask anything about your strategy or growth. Get specific answers." }
            ].map(function(f) {
              return (
      ...
      <div style={{ minHeight:"100vh", background:BRAND.bg, fontFamily:"'Helvetica Neue',Helvetica,Arial,sans-serif", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
        <div style={{ maxWidth:440, width:"100%" }}>
          <div style={{ textAlign:"center", marginBottom:32 }}>
            ...
            <div style={{ fontSize:15, color:BRAND.muted, marginTop:8 }}>
              {authMode === "signup" ? "Create your free account to get started" : "Welcome back — sign in to your account"}
            </div>
          </div>
          <div style={cardSt}>
  ...
  if (screen === "onboarding") {
    ...
    function toggle(arr, val, max) {
      max = max || 99;
      if (arr.indexOf(val) !== -1) return arr.filter(function(x){ return x !== val; });
      if (arr.length >= max) return arr;
      return arr.concat([val]);
    }
    var stepOk = [
      !!(profile.name && profile.job),
      profile.platforms.length === 3,   // exactly 3
        ...
        <div style={{ maxWidth:540, margin:"40px auto", padding:"0 24px" }}>
          ...
          <div style={cardSt}>
            {obStep === 0 && (
              <div>
                ...
                <input style={{ ...inputSt, marginBottom:14 }} placeholder="e.g. Sarah" value={profile.name} onChange={function(e){ setProfile(function(p){ return Object.assign({},p,{name:e.target.value}); }); }} />
                <span style={labelSt}>What do you do?</span>
                <input style={inputSt} placeholder="e.g. Fitness coach, designer, entrepreneur..." value={profile.job} onChange={function(e){ setProfile(function(p){ return Object.assign({},p,{job:e.target.value}); }); }} />
              </div>
            )}
            {obStep === 1 && (
              <div>
                <div style={{ fontSize:22, fontWeight:800, marginBottom:6 }}>Pick your top 3 platforms</div>
                <div style={{ fontSize:13, color:BRAND.muted, marginBottom:6 }}>Select exactly 3 platforms you want to create content for</div>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
                  {[1,2,3].map(function(n) {
                    var filled = profile.platforms.length >= n;
                    return <div key={n} style={{ width:28, height:28, borderRadius:"50%", background:filled?BRAND.black:BRAND.bg, border:`2px solid ${filled?BRAND.black:BRAND.border}`, display:"flex", alignItems:"center", justifyContent:"center" }}><span style={{ color:filled?BRAND.white:BRAND.border, fontSize:12, fontWeight:700 }}>{n}</span></div>;
                  })}
                ...
                <div>
                  {PLATFORMS.map(function(pl) {
                    ...
                    var atLimit = profile.platforms.length >= 3;
                    return (
                      <span key={pl} style={{ ...tagSt(on), opacity:!on&&atLimit?0.35:1, cursor:!on&&atLimit?"not-allowed":"pointer" }} onClick={function(){
                        if (!on && atLimit) return;
                        setProfile(function(p){ return Object.assign({},p,{platforms:toggle(p.platforms,pl,3)}); });
                      }}>{pl}</span>
                    );
                  })}
                </div>
                {profile.platforms.length === 3 && (
                  <div style={{ marginTop:16, padding:"10px 14px", background:"#f0fff8", border:"1px solid #c8f5e0", fontSize:13, color:"#2d7a4f", fontWeight:600 }}>
                    ✓ {profile.platforms.join(", ")} selected
                  </div>
                )}
              </div>
            )}
            {obStep === 2 && (
              <div>
                <div style={{ fontSize:22, fontWeight:800, marginBottom:6 }}>Why are you creating content?</div>
                <div style={{ fontSize:13, color:BRAND.muted, marginBottom:14 }}>Pick up to 3 goals</div>
                <div>
                  {GOALS.map(function(g) {
                    var on = profile.goals.indexOf(g) !== -1;
                    return <span key={g} style={tagSt(on)} onClick={function(){ setProfile(function(p){ return Object.assign({},p,{goals:toggle(p.goals,g,3)}); }); }}>{g}</span>;
                  })}
                </div>
              </div>
            )}
            {obStep === 3 && (
              <div>
                ...
                <div style={{ fontSize:13, color:BRAND.muted, marginBottom:14 }}>Your story, values, and who you are talking to <span style={{ opacity:0.6 }}>(optional)</span></div>
                <textarea style={{ ...inputSt, height:130, resize:"vertical" }} placeholder="e.g. I left corporate to become a life coach. I help women 30-45 find confidence and clarity..." value={profile.brand} onChange={function(e){ setProfile(function(p){ return Object.assign({},p,{brand:e.target.value}); }); }} />
              </div>
            )}
            {obStep === 4 && (
              <div>
                ...
                <div>
                  {VIBES.map(function(v) {
                    ...
                    return <span key={v} style={tagSt(on)} onClick={function(){ setProfile(function(p){ return Object.assign({},p,{vibe:v}); }); }}>{v}</span>;
                  })}
                </div>
              </div>
            )}
          ...
          <div style={{ display:"flex", justifyContent:"space-between", gap:12 }}>
            ...
            {obStep < steps.length-1
              ...
              : <button style={{ ...rainbowBtn, marginLeft:"auto" }} onClick={generateStrategy}>{loading?"Building your strategy...":"Generate My Strategy →"}</button>
            }
          </div>
        </div>
      </div>
  ...
  if (screen === "upgrade") {
    var plans = [
      { name:"Free", price:"$0", sub:"Forever free", features:["15 AI generations / month","3 platforms","Full onboarding and strategy"], cta:"Current Plan", pro:false },
  ...
  function renderSection() {
    ...
    if (nav === 0) {
      return (
        <div>
          {!isPro && (
            <div style={{ ...cardSt, background:"#f9f9f9", borderLeft:`3px solid #4ecdc4`, marginBottom:16, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
              <div>
                <div style={{ fontWeight:700, fontSize:14, marginBottom:2 }}>Free Plan — {gens}/{FREE_LIMIT} generations used</div>
                <div style={{ fontSize:12, color:BRAND.muted }}>Upgrade for unlimited access across all platforms and sections</div>
              </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
            {[{l:"Ideas",v:ideas.length},{l:"Picked",v:picked.length},{l:"Months",v:monthPlans.filter(Boolean).length},{l:"Saved Chats",v:savedChats.length}].map(function(st) {
              return (
                <div key={st.l} style={{ ...cardSt, textAlign:"center", marginBottom:0 }}>
          ...
          {monthPlans[curMonth] ? (
            <div>
              ...
              {monthPlans[curMonth].weeklyThemes && monthPlans[curMonth].weeklyThemes.map(function(w, i) {
                return (
                  <div key={i} style={{ display:"flex", gap:20, padding:"12px 0", borderBottom:`1px solid ${BRAND.border}`, fontSize:14 }}>
            ...
            <div>
              ...
              <input style={{ ...inputSt, marginBottom:14 }} placeholder="e.g. launching my new service, behind the scenes..." value={monthInput} onChange={function(e){ setMonthInput(e.target.value); }} />
    ...
    if (nav === 4) {
      return (
        <div style={cardSt}>
          ...
          {picked.length === 0 ? <div style={{ color:BRAND.muted }}>No ideas picked yet. Go to Ideas first.</div> : (
            <div>
      ...
      return (
        <div>
          {picked.length === 0 ? <div style={{ ...cardSt, color:BRAND.muted }}>Pick some ideas first.</div> : picked.map(function(idea, i) {
            ...
            var pc = pillarColor(idea.pillar);
            return (
              <div key={i} style={{ ...cardSt, borderLeft:`4px solid ${pc}` }}>
  ...
  return (
    <div style={{ minHeight:"100vh", background:BRAND.bg, fontFamily:"'Helvetica Neue',Helvetica,Arial,sans-serif", color:BRAND.black }}>
      {showLimitModal && (
        <LimitModal
          resetDate={gensResetDate}
          onUpgrade={function(){ setShowLimitModal(false); setScreen("upgrade"); }}
          onClose={function(){ setShowLimitModal(false); }}
        />
      )}
      ...
      <div style={{ background:BRAND.white, borderBottom:`1px solid ${BRAND.border}`, padding:"0 32px", display:"flex", overflowX:"auto" }}>
        {NAV_LABELS.map(function(label, i) {
          var locked = !pack && i > 0;
          return <button key={i} style={navSt(i===nav, locked)} onClick={function(){ if(!locked) setNav(i); }}>{label}</button>;
        })}
      </div>
