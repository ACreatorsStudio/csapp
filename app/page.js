...
export default function App() {
  ...
  // ── STRIPE ─────────────────────────────────────────────
  async function startCheckout(billingType) {
  ...
  async function openPortal() {
    try {
      var res = await fetch("/api/stripe/portal", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ email:user&&user.email }) });
      var data = await res.json();
      if (data.url) { window.location.href = data.url; }
  ...
  function useGen(updates) {
    ...
    var n = gens+1, resetDate = gensResetDate;
  ...
  async function generateStrategy() {
    ...
    try {
      ...
      var data = parseJSON(raw?.content?.[0]?.text||"");
  ...
  async function sendChat() {
    ...
    try {
      var pillars=pack&&pack.pillars?pack.pillars.map(function(p){return p.name;}).join(","):"";
      var txt=await callAI(nm,"You are a content strategy advisor for "+profile.name+"("+profile.job+"). Angle:"+(pack?pack.angle:"")+". Pillars:"+pillars+". Be specific.");
  ...
  function onDragStart(e, idea, fromSlot) {
    e.preventDefault();
    setDraggedIdea(idea); setDraggedFromSlot(fromSlot); setIsDragging(true);
    ...
    function onEnd(ev) {
      ...
      setSlots(function(prev){
        ...
        if(targetSlot){if(fromSlot&&fromSlot!==targetSlot)delete ns[fromSlot];ns[targetSlot]=idea;}
        else if(poolEl&&fromSlot){delete ns[fromSlot];}
        saveUserData({slots:ns});return ns;
      });
      setDraggedIdea(null);setDraggedFromSlot(null);setDragOverSlot(null);setIsDragging(false);
    ...
    return (
      <div style={{minHeight:"100vh",background:BRAND.bg,fontFamily:"'Helvetica Neue',Helvetica,Arial,sans-serif",color:BRAND.black}}>
        ...
        <div style={{maxWidth:700,margin:"60px auto",padding:"0 24px"}}>
          ...
          <div style={{color:BRAND.muted,marginBottom:16}}>Start free — no credit card required for trial</div>
          <div style={{padding:"14px 20px",background:BRAND.black,color:BRAND.white,marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
            <div>
              <div style={{fontWeight:700,fontSize:14}}>🎁 VIP Early Access — 3 months free</div>
              <div style={{fontSize:12,opacity:0.6,marginTop:2}}>Got an invite code? Enter it below.</div>
            </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            {plans.map(function(pl){
              return (
                <div key={pl.name} style={{...cardSt,border:pl.pro?"2px solid "+BRAND.black:undefined,position:"relative"}}>
          ...
          {isPro&&<div style={{marginTop:24,textAlign:"center"}}><button style={{...outlineBtn,fontSize:13}} onClick={openPortal}>Manage Subscription / Cancel →</button></div>}
        </div>
      </div>
    );
  }
  ...
  function renderSection() {
        ...
        {picked.length===0?<div style={{...cardSt,color:BRAND.muted}}>Pick some ideas first.</div>:picked.map(function(idea,i){
          ...
          return <div key={i} style={{...cardSt,borderLeft:"4px solid "+pc}}>
            ...
            {data&&<div>
              ...
              {data.hooks&&data.hooks.map(function(h,j){return <EditableBlock key={j} label={h.type} text={h.text} onSave={function(t){var hs=data.hooks.slice();hs[j]=Object.assign({},h,{text:t});var u=Object.assign({},hooks);u[idea.title]=Object.assign({},data,{hooks:hs});setHooks(u);saveUserData({hooks:u});}}/>;})}<div style={divider}/>
    ...
    if (nav===7) return (
      <div style={{position:"relative"}}>
        {isDragging&&draggedIdea&&<div style={{position:"fixed",left:dragPos.x+12,top:dragPos.y+12,zIndex:9999,pointerEvents:"none",padding:"8px 14px",background:pillarColor(draggedIdea.pillar),color:"#fff",fontSize:12,fontWeight:700,maxWidth:180,boxShadow:"0 8px 24px rgba(0,0,0,0.3)",opacity:0.95}}>{draggedIdea.title}</div>}
        <div style={{...cardSt,marginBottom:12}}><div style={{fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",color:BRAND.muted,marginBottom:4}}>4-Week Content Scheduler</div><div style={{fontSize:13,color:BRAND.muted}}>Drag ideas from the pool below onto the calendar slots</div></div>
        <div data-pool="true" style={{...cardSt,marginBottom:12}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {unscheduled.map(function(idea,i){var pc=pillarColor(idea.pillar);return <div key={i} onMouseDown={function(e){onDragStart(e,idea,null);}} onTouchStart={function(e){onDragStart(e,idea,null);}} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",background:BRAND.white,border:"2px solid "+pc,cursor:"grab",userSelect:"none",WebkitUserSelect:"none",fontSize:13,fontWeight:600}}><span style={{width:8,height:8,borderRadius:"50%",background:pc,display:"inline-block",flexShrink:0}}/><span style={{maxWidth:160,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{idea.title}</span><span style={{fontSize:10,color:BRAND.muted}}>{idea.format}</span></div>;})}
          </div>
        </div>
        {WEEKS.map(function(week){return <div key={week} style={{...cardSt,marginBottom:10}}><div style={{fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",marginBottom:10}}>WEEK {week}</div><div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:6}}>{DAYS.map(function(day){var key="w"+week+"_"+day,slotIdea=slots[key],isOver=dragOverSlot===key,pc=slotIdea?pillarColor(slotIdea.pillar):null;return <div key={day} data-slot={key} style={{minHeight:96,border:"2px "+(isOver?"solid":"dashed")+" "+(isOver?"#4ecdc4":BRAND.border),background:isOver?"#e8fffe":slotIdea?pc+"18":"#fafafa",padding:6,boxSizing:"border-box"}}><div style={{fontSize:10,fontWeight:700,letterSpacing:0.5,textTransform:"uppercase",color:isOver?"#4ecdc4":BRAND.muted,marginBottom:4}}>{day}</div>{slotIdea?<div onMouseDown={function(e){onDragStart(e,slotIdea,key);}} onTouchStart={function(e){onDragStart(e,slotIdea,key);}} style={{padding:"6px 7px",background:pc,cursor:"grab",userSelect:"none",WebkitUserSelect:"none",position:"relative"}}><div style={{fontSize:10,fontWeight:700,color:"#fff",lineHeight:1.3,marginBottom:2,overflow:"hidden"}}>{slotIdea.title}</div><div style={{fontSize:9,color:"rgba(255,255,255,0.8)"}}>{slotIdea.format}</div><button onMouseDown={function(e){e.stopPropagation();}} onClick={function(e){e.stopPropagation();var ns=Object.assign({},slots);delete ns[key];setSlots(ns);saveUserData({slots:ns});}} style={{position:"absolute",top:2,right:2,background:"rgba(0,0,0,0.4)",color:"#fff",border:"none",cursor:"pointer",fontSize:10,width:16,height:16,display:"flex",alignItems:"center",justifyContent:"center",padding:0}}>×</button></div>:<div style={{fontSize:9,color:BRAND.border,textAlign:"center",marginTop:18,pointerEvents:"none"}}>drop here</div>}</div>;})};</div></div>;})}
    ...
    if (nav===8) {
      var weekGroups={};
      WEEKS.forEach(function(week){var items=[];DAYS.forEach(function(day){var k="w"+week+"_"+day;if(slots[k])items.push({idea:slots[k],day:day,slotKey:k});});if(items.length>0)weekGroups[week]=items;});
      ...
      return (
        <div>
          ...
          {!hasContent&&<div style={{...cardSt,textAlign:"center",padding:40}}><div style={{fontSize:32,marginBottom:12}}>📅</div><div style={{fontWeight:700,fontSize:16,marginBottom:8}}>No content scheduled yet</div><div style={{color:BRAND.muted,fontSize:14,marginBottom:20}}>Go to the Scheduler and drag your ideas onto the calendar first.</div><button style={rainbowBtn} onClick={function(){setNav(7);}}>Go to Scheduler →</button></div>}
          {WEEKS.map(function(week){
            ...
            return <div key={week} style={{marginBottom:24}}>
              <div style={{display:"flex",alignItems:"baseline",gap:12,marginBottom:12}}><div style={{fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>WEEK {week}</div>{weekTheme&&<div style={{fontSize:13,color:BRAND.muted,fontStyle:"italic"}}>{weekTheme}</div>}</div>
              {weekGroups[week].map(function(entry,i){
                var idea=entry.idea,pc=pillarColor(idea.pillar),plan=shoots[idea.title],hookData=hooks[idea.title];
                ...
                var pillarObj=pack&&pack.pillars?pack.pillars.find(function(p){return p.name===idea.pillar;}):null;
                return <div key={i} style={{...cardSt,borderLeft:"4px solid "+pc,marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,flexWrap:"wrap",gap:8}}>
                    ...
                    <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                      {STATUS_STEPS.map(function(st){var isActive=status===st,isDone=STATUS_STEPS.indexOf(status)>STATUS_STEPS.indexOf(st);return <button key={st} style={{padding:"4px 12px",fontSize:11,fontWeight:700,cursor:"pointer",border:"1.5px solid "+(isActive||isDone?STATUS_COLORS[st]:BRAND.border),background:isActive?STATUS_COLORS[st]:isDone?STATUS_COLORS[st]+"30":"transparent",color:isActive?BRAND.white:isDone?STATUS_COLORS[st]:BRAND.muted,borderRadius:20}} onClick={function(){var ns=Object.assign({},postStatus);ns[idea.title]=st;setPostStatus(ns);saveUserData({postStatus:ns});}}>{isDone||isActive?"✓ ":""}{st}</button>;})}
                    </div>
                  </div>
                  {firstHook&&<div style={{marginBottom:12}}><div style={{fontSize:10,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:BRAND.muted,marginBottom:6}}>Hook — {firstHook.type}</div><div style={{padding:12,background:"#f9f9f9",border:"1px solid "+BRAND.border,fontSize:14,lineHeight:1.6,display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12}}><div style={{flex:1}}>{firstHook.text}</div><button style={{fontSize:11,fontWeight:700,padding:"4px 12px",background:BRAND.black,color:BRAND.white,border:"none",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}} onClick={function(){navigator.clipboard.writeText(firstHook.text);}}>Copy</button></div></div>}
                  {firstCaption&&<div style={{marginBottom:12}}><div style={{fontSize:10,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:BRAND.muted,marginBottom:6}}>Caption</div><div style={{padding:12,background:"#f9f9f9",border:"1px solid "+BRAND.border,fontSize:14,lineHeight:1.6,display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12}}><div style={{flex:1,whiteSpace:"pre-wrap"}}>{firstCaption.text}{firstCaption.cta?"\n\n"+firstCaption.cta:""}</div><button style={{fontSize:11,fontWeight:700,padding:"4px 12px",background:BRAND.black,color:BRAND.white,border:"none",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}} onClick={function(){navigator.clipboard.writeText(firstCaption.text+(firstCaption.cta?"\n\n"+firstCaption.cta:""));}}>Copy</button></div></div>}
                  {plan&&plan.shotList&&plan.shotList.length>0&&<div style={{marginBottom:12}}><div style={{fontSize:10,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:BRAND.muted,marginBottom:6}}>Shot List</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{plan.shotList.map(function(sh,j){return <span key={j} style={{fontSize:12,padding:"4px 10px",background:"#f0f0f0",border:"1px solid "+BRAND.border}}>— {sh}</span>;})}</div></div>}
                  {plan&&plan.script&&<div><div style={{fontSize:10,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:BRAND.muted,marginBottom:6}}>Script Preview</div><div style={{padding:12,background:"#f9f9f9",border:"1px solid "+BRAND.border,fontSize:13,lineHeight:1.6,color:BRAND.muted,maxHeight:80,overflow:"hidden",position:"relative"}}>{plan.script.substring(0,200)}{plan.script.length>200?"...":""}<div style={{position:"absolute",bottom:0,left:0,right:0,height:30,background:"linear-gradient(transparent,#f9f9f9)"}}/></div><button style={{fontSize:11,color:BRAND.muted,background:"transparent",border:"none",cursor:"pointer",padding:"4px 0",textDecoration:"underline"}} onClick={function(){setNav(5);}}>View full script →</button></div>}
                  {(!plan||!hookData)&&<div style={{marginTop:12,padding:10,background:"#fff8f0",border:"1px solid #ffd4a0",fontSize:13,color:"#c07000"}}>⚠️ Missing: <span style={{cursor:"pointer",textDecoration:"underline"}} onClick={function(){setNav(!plan?5:6);}}>Generate {!plan?"shoot plan":""}{!plan&&!hookData?" and ":""}{!hookData?"hooks":""}</span></div>}
                </div>;
              })}
            </div>;
          })}
    ...
    if (nav===10) return (
      <div style={cardSt}>
        ...
        {savedChats.length>0&&<div style={{marginTop:18}}><div style={{fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:BRAND.muted,marginBottom:10}}>Saved Conversations</div>{savedChats.map(function(c,i){return <div key={i} style={{padding:10,border:"1px solid "+BRAND.border,marginBottom:8,cursor:"pointer",background:"#f9f9f9"}} onClick={function(){setChat(c.msgs);}}><div style={{fontWeight:600,fontSize:13}}>Conversation {i+1}</div><div style={{fontSize:12,color:BRAND.muted}}>{c.date} — {c.msgs.length} messages</div></div>;})}</div>}
      </div>
    );
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"28px 20px"}}>
        {pack&&<div style={{marginBottom:20}}>
          <div style={{display:"flex",gap:4,marginBottom:12,alignItems:"center"}}>
            {JOURNEY.slice(1).map(function(j,idx){var done=nav>j.nav,active=nav===j.nav;return <div key={idx} style={{display:"flex",alignItems:"center"}}><div style={{width:active?28:8,height:8,borderRadius:4,background:active?BRAND.rainbow:done?"#4ecdc4":BRAND.border,transition:"all 0.3s",cursor:"pointer"}} onClick={function(){setNav(j.nav);}}/></div>;})}
