...
function EditableBlock({ label, text, onSave }) {
  ...
  const btn = (dark) => ({
    background:dark?BRAND.black:"transparent",
    color:dark?BRAND.white:BRAND.black,
    border:`1px solid ${dark?BRAND.black:BRAND.border}`,
...
export default function App() {
  ...
  async function handleEmailAuth() {
    ...
    try {
      if(authMode==="signup") {
        ...
        if(data.user){ setUser(data.user); setShowAuth(false); await generateStrategy(data.user); }
      } else {
  ...
  function useGen(updates={}) {
    ...
    const n=gens+1; setGens(n); saveUserData({...updates,gens:n}); return true;
  }

  // ── Active platforms — always derived safely ──────────────
  const activePlatforms = isPro
    ? profile.platforms
    : (profile.activePlatforms?.length ? profile.activePlatforms : profile.platforms.slice(0,3));
  ...
  async function genShoot(idea) {
    ...
    try {
      const txt = await callAI([{role:"user",content:`Shoot plan for:"${idea.title}"(${idea.format}) on ${idea.platform}. Creator:${profile.name},${profile.job}. Vibe:${profile.vibe}. Return ONLY JSON:{"script":"full script","shotList":["..."],"repurpose":["..."]}`}],"Return only valid JSON, no markdown.");
      const data = parseJSON(txt);
  ...
  if(screen==="onboarding") {
    ...
    // ✅ FIX: each step only checks its own required fields
    const stepOk=[
      !!(profile.name&&profile.job),          // step 0
      profile.platforms.length>0,              // step 1 — just needs ≥1 platform
      profile.goals.length>0,                  // step 2
      true,                                     // step 3 — brand desc optional
      !!profile.vibe                            // step 4
    ];
    const ok = stepOk[obStep];
    return (
      <div style={S.page}>
        ...
        <div style={{maxWidth:540,margin:"40px auto",padding:"0 24px"}}>
          ...
          <div style={S.card}>
            {obStep===0&&<>
              ...
              <input style={{...S.input,marginBottom:14}} placeholder="e.g. Sarah" value={profile.name} onChange={e=>setProfile(p=>({...p,name:e.target.value}))}/>
              <span style={S.label}>What do you do?</span>
              <input style={S.input} placeholder="e.g. Fitness coach, designer..." value={profile.job} onChange={e=>setProfile(p=>({...p,job:e.target.value}))}/>
            </>}
            {obStep===1&&<>
              ...
              <div>{PLATFORMS.map(pl=><span key={pl} style={S.tag(profile.platforms.includes(pl))} onClick={()=>setProfile(p=>({...p,platforms:toggle(p.platforms,pl)}))}>{pl}</span>)}</div>
            </>}
            {obStep===2&&<>
              ...
              <div>{GOALS.map(g=><span key={g} style={S.tag(profile.goals.includes(g))} onClick={()=>setProfile(p=>({...p,goals:toggle(p.goals,g,3)}))}>{g}</span>)}</div>
            </>}
            {obStep===3&&<>
              ...
              <textarea style={{...S.input,height:130,resize:"vertical"}} placeholder="e.g. I'm a mom of 2 who left corporate..." value={profile.brand} onChange={e=>setProfile(p=>({...p,brand:e.target.value}))}/>
            </>}
            {obStep===4&&<>
              ...
              <div>{VIBES.map(v=><span key={v} style={S.tag(profile.vibe===v)} onClick={()=>setProfile(p=>({...p,vibe:v}))}>{v}</span>)}</div>
            </>}
          </div>
          <div style={{display:"flex",justifyContent:"space-between",gap:12}}>
            {obStep>0&&<button style={S.outBtn} onClick={()=>setObStep(s=>s-1)}>← Back</button>}
            {obStep<steps.length-1
              ?<button style={{...S.bigBtn(ok),marginLeft:"auto"}} disabled={!ok} onClick={()=>{ if(ok) setObStep(s=>s+1); }}>Next →</button>
              :<button style={{...S.rainbowBtn,marginLeft:"auto"}} onClick={()=>{ if(user){generateStrategy(user);}else{setShowAuth(true);} }}>
                {loading?"Building...":"Save & Generate Strategy →"}
              </button>
            }
          </div>
  ...
  function SectionHome() {
    const showWarning = !isPro && profile.platforms.length > 3;
    const currentActive = profile.activePlatforms?.length
      ? profile.activePlatforms
      : profile.platforms.slice(0,3);

    function togglePlatform(pl) {
      const active = currentActive.includes(pl);
      if(!active && currentActive.length>=3) return;
      const upd = active ? currentActive.filter(x=>x!==pl) : [...currentActive,pl];
      const np={...profile,activePlatforms:upd}; setProfile(np); saveUserData({profile:np});
    }
          ...
          <div style={{...S.card,borderLeft:`4px solid #f4845f`,background:"#fff8f5",marginBottom:16}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:12}}>
              {profile.platforms.map(pl=>{
                const active=currentActive.includes(pl);
                const atLimit=currentActive.length>=3;
                return(<span key={pl} style={{...S.tag(active),opacity:!active&&atLimit?0.35:1,cursor:!active&&atLimit?"not-allowed":"pointer"}} onClick={()=>togglePlatform(pl)}>{pl}</span>);
              })}
            </div>
            <div style={{fontSize:13,color:BRAND.muted}}>Active: <strong>{currentActive.join(", ")}</strong></div>
            {currentActive.length<3&&<div style={{fontSize:12,color:"#f4845f",marginTop:6}}>Please select {3-currentActive.length} more platform{3-currentActive.length>1?"s":""}</div>}
