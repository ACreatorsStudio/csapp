...
// ← Points to your secure backend route instead of Anthropic directly
...
function EditableBlock({ label, text, onSave }) {
  ...
  const btn = (dark) => ({ background:dark?BRAND.black:"transparent", color:dark?BRAND.white:BRAND.black, border:`1px solid ${dark?BRAND.black:BRAND.border}`, padding:"6px 14px", fontSize:11, fontWeight:700, cursor:"pointer", letterSpacing:0.5 });
  return (
...
export default function App() {
  ...
  useEffect(()=>{
    try {
      const s = localStorage.getItem("cs_v2");
      if(!s) return;
      const d = JSON.parse(s);
      if(d.profile) setProfile(d.profile);
      if(d.pack){ setPack(d.pack); setScreen("app"); }
      if(d.monthPlans) setMonthPlans(d.monthPlans);
      ...
      if(d.savedChats) setSavedChats(d.savedChats);
  ...
  function save(u) {
    try {
      const cur = JSON.parse(localStorage.getItem("cs_v2")||"{}");
      localStorage.setItem("cs_v2", JSON.stringify({...cur,...u}));
    } catch(e){}
  }

  function canGen() { return isPro || gens < FREE_LIMIT; }
  function useGen() { if(!canGen()) return false; const n=gens+1; setGens(n); save({gens:n}); return true; }
  ...
  const S = {
    ...
    smBtn(on=true){ return { background:on?BRAND.black:BRAND.bg, color:on?BRAND.white:BRAND.muted, border:`1px solid ${on?BRAND.black:BRAND.border}`, padding:"8px 14px", fontWeight:600, fontSize:12, cursor:"pointer", letterSpacing:0.3 }; },
  ...
  async function genStrategy() {
    ...
    try {
      ...
      if(data){
        ...
        setPack(data); setProfile(np); save({pack:data, profile:np}); useGen();
      }
  ...
  async function genMonthPlan(idx) {
    ...
    try {
      ...
      const data = parseJSON(txt);
      if(data){ const u=[...monthPlans]; u[idx]={...data,userInput:monthInput||"growth"}; setMonthPlans(u); save({monthPlans:u}); useGen(); }
    } catch(e){}
  ...
  async function genIdeas() {
    ...
    try {
      ...
      const data = parseJSON(txt);
      if(data){ setIdeas(data); save({ideas:data}); useGen(); }
    } catch(e){}
    setLoading("");
  }
  ...
  async function genShoot(idea) {
    ...
    setLoading("shoot_"+key);
    try {
      const txt = await callAI([{role:"user",content:`Create a shoot plan for this content piece:\nTitle: "${idea.title}"\nFormat: ${idea.format}\nPlatform: ${idea.platform}\nCreator: ${profile.name}, ${profile.job}\nVibe: ${profile.vibe}\n\nReturn ONLY a JSON object like this:\n{"script":"the full script or talking points here","shotList":["shot 1","shot 2","shot 3"],"repurpose":["repurpose idea 1","repurpose idea 2"]}`}],"Return only valid JSON, no markdown, no explanation.");
      const data = parseJSON(txt);
      if(data && data.script){ const u={...shoots,[key]:data}; setShoots(u); save({shoots:u}); useGen(); }
      else { const u={...shoots,[key]:{script:txt||"Could not generate. Try again.",shotList:[],repurpose:[]}}; setShoots(u); save({shoots:u}); }
    } catch(e){ const u={...shoots,[idea.title]:{script:"Error — please try again.",shotList:[],repurpose:[]}}; setShoots(u); save({shoots:u}); }
    setLoading("");
  }
  ...
  async function genHooks(idea) {
    ...
    try {
      ...
      const data = parseJSON(txt);
      if(data){ const u={...hooks,[idea.title]:data}; setHooks(u); save({hooks:u}); useGen(); }
      else { const u={...hooks,[idea.title]:{hooks:[{type:"Error",text:"Failed — please retry"}],captions:[{platform:idea.platform,text:"Try again",cta:""}]}}; setHooks(u); }
    } catch(e){ const u={...hooks,[idea.title]:{hooks:[{type:"Error",text:"Failed — please retry"}],captions:[]}}; setHooks(u); }
    setLoading("");
  }
  ...
  async function sendChat() {
    ...
    try {
      ...
      const fm=[...nm,{role:"assistant",content:txt}];
      setChat(fm); save({chat:fm}); useGen();
    } catch(e){}
    setLoading("");
    ...
    function onUp(ev) {
      ...
      if(slotKey) {
        setSlots(prev => {
          ...
          save({slots:ns}); return ns;
        });
      } else if(dragRef.current.fromSlot && el?.closest("[data-pool]")) {
        setSlots(prev => { const ns={...prev}; delete ns[dragRef.current.fromSlot]; save({slots:ns}); return ns; });
      }
      ...
      <div style={S.page}>
        ...
        <div style={{maxWidth:540,margin:"40px auto",padding:"0 24px"}}>
          <div style={{display:"flex",border:`1px solid ${BRAND.border}`,marginBottom:28}}>
            {steps.map((st,i)=>(
              <div key={i} style={{flex:1,padding:"10px 0",textAlign:"center",fontSize:11,fontWeight:700,letterSpacing:0.5,textTransform:"uppercase",background:i<obStep?BRAND.black:i===obStep?BRAND.white:BRAND.bg,color:i<obStep?BRAND.white:i===obStep?BRAND.black:BRAND.border,borderRight:i<steps.length-1?`1px solid ${BRAND.border}`:"none"}}>{st}</div>
            ))}
          </div>
          <div style={S.card}>
            ...
            {obStep===1&&<><div style={{fontSize:22,fontWeight:800,marginBottom:6}}>Which platforms?</div><div style={{fontSize:13,color:BRAND.muted,marginBottom:14}}>Select all you want — you can refine later</div><div>{PLATFORMS.map(pl=><span key={pl} style={S.tag(profile.platforms.includes(pl))} onClick={()=>setProfile(p=>({...p,platforms:toggle(p.platforms,pl)}))}>{pl}</span>)}</div></>}
            ...
            {obStep===3&&<><div style={{fontSize:22,fontWeight:800,marginBottom:6}}>Describe your brand</div><div style={{fontSize:13,color:BRAND.muted,marginBottom:14}}>Your story, values, and audience</div><textarea style={{...S.input,height:130,resize:"vertical"}} placeholder="e.g. I'm a mom of 2 who left corporate to become a life coach..." value={profile.brand} onChange={e=>setProfile(p=>({...p,brand:e.target.value}))}/></>}
          ...
          <div style={{display:"flex",justifyContent:"space-between",gap:12}}>
            ...
            {obStep<steps.length-1
              ...
              :<button style={{...S.rainbowBtn,marginLeft:"auto"}} onClick={genStrategy}>{loading?"Building strategy...":"Generate My Strategy →"}</button>
            }
          </div>
        </div>
      </div>
    ...
    return (
      <div style={S.page}>
        ...
        <div style={{maxWidth:700,margin:"60px auto",padding:"0 24px"}}>
          <div style={{fontSize:11,fontWeight:700,letterSpacing:2,color:BRAND.muted,textTransform:"uppercase",marginBottom:12}}>Upgrade</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            {plans.map(pl=>(
              <div key={pl.name} style={{...S.card,border:pl.pro?`2px solid ${BRAND.black}`:undefined,position:"relative"}}>
                ...
                <button style={{...S.bigBtn(pl.pro),width:"100%",marginTop:20,textAlign:"center"}} onClick={()=>{if(pl.pro){setIsPro(true);save({isPro:true});setScreen("app");}}}>{pl.cta}</button>
              </div>
            ))}
          </div>
        </div>
    ...
    return (
      <div>
          ...
          <div style={{...S.card,borderLeft:`4px solid #f4845f`,background:"#fff8f5",marginBottom:16}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:12}}>
              {profile.platforms.map(pl=>{
                ...
                return(<span key={pl} style={{...S.tag(active),opacity:!active&&atLimit?0.35:1,cursor:!active&&atLimit?"not-allowed":"pointer"}} onClick={()=>{if(!active&&atLimit)return;const cur=profile.activePlatforms||[];const upd=active?cur.filter(x=>x!==pl):[...cur,pl];const np={...profile,activePlatforms:upd};setProfile(np);save({profile:np});}}>{pl}</span>);
              })}
            </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
          {[{l:"Ideas",v:ideas.length},{l:"Picked",v:picked.length},{l:"Months",v:monthPlans.filter(Boolean).length},{l:"Saved Chats",v:savedChats.length}].map(st=>(
            <div key={st.l} style={{...S.card,textAlign:"center",marginBottom:0}}>
        ...
        {!pack?<div style={{color:BRAND.muted}}>Complete onboarding first.</div>:<>
          ...
          {pack.pillars?.map((p,i)=>(
            <div key={i} style={{display:"flex",gap:16,alignItems:"center",padding:"14px 0",borderBottom:`1px solid ${BRAND.border}`}}>
              ...
              <div><div style={{fontWeight:800,fontSize:15,color:PILLAR_COLORS[i]}}>{p.name}</div><div style={{fontSize:13,color:BRAND.muted,marginTop:3,lineHeight:1.5}}>{p.description}</div></div>
            </div>
          ))}
    ...
    return (
      <div style={S.card}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:18,padding:14,background:"#f9f9f9",border:`1px solid ${BRAND.border}`}}>
            ...
            {pack.pillars.map((p,i)=>(
              <span key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"4px 12px",background:PILLAR_COLORS[i]+"20",border:`1.5px solid ${PILLAR_COLORS[i]}`,fontSize:12,fontWeight:700,color:PILLAR_COLORS[i]}}>
                <span style={{width:8,height:8,borderRadius:"50%",background:PILLAR_COLORS[i],display:"inline-block"}}/>{p.emoji} {p.name}
              </span>
            ))}
          </div>
        )}
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:18}}>
          ...
          {[5,10,15,20,30].map(n=><button key={n} style={S.smBtn(n===ideaCount)} onClick={()=>setIdeaCount(n)}>{n}</button>)}
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div style={{flex:1,paddingRight:12}}>
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,flexWrap:"wrap"}}>
                          <span style={{fontSize:11,fontWeight:700,padding:"2px 10px",background:pc+"20",color:pc,border:`1px solid ${pc}`,letterSpacing:0.3}}>{pack?.pillars?.find(p=>p.name===idea.pillar)?.emoji} {idea.pillar}</span>
                          <span style={{fontSize:11,color:BRAND.muted,letterSpacing:0.3,textTransform:"uppercase"}}>{idea.platform} · {idea.format}</span>
                        </div>
                      ...
                      <button style={S.smBtn(isPicked)} onClick={()=>{const has=picked.find(p=>p.title===idea.title);const u=has?picked.filter(p=>p.title!==idea.title):[...picked,idea];setPicked(u);save({picked:u});}}>{isPicked?"✓ Picked":"Pick"}</button>
                    </div>
                  </div>
                </div>
              );
            })}
            {picked.length>0&&<button style={{...S.bigBtn(),marginTop:12}} onClick={()=>setNav(4)}>View Picked ({picked.length}) →</button>}
          </div>
        )}
      </div>
    );
    ...
    return (
      <div style={S.card}>
        ...
        {picked.length===0?<div style={{color:BRAND.muted}}>No ideas picked yet.</div>:<>
          {picked.map((idea,i)=>{const pc=pillarColor(idea.pillar);return(<div key={i} style={{display:"flex",alignItems:"stretch",marginBottom:8,border:`1px solid ${BRAND.border}`,overflow:"hidden"}}><div style={{width:6,background:pc,flexShrink:0}}/><div style={{flex:1,padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><span style={{fontSize:11,fontWeight:700,padding:"2px 10px",background:pc+"20",color:pc,border:`1px solid ${pc}`}}>{pack?.pillars?.find(p=>p.name===idea.pillar)?.emoji} {idea.pillar}</span><div style={{fontWeight:700,fontSize:14,marginTop:8}}>{idea.title}</div><div style={{fontSize:11,color:BRAND.muted,letterSpacing:0.5,textTransform:"uppercase",marginTop:3}}>{idea.platform} · {idea.format}</div></div><button style={{...S.smBtn(false),fontSize:11}} onClick={()=>{const u=picked.filter(p=>p.title!==idea.title);setPicked(u);save({picked:u});}}>Remove</button></div></div>);})}
          <button style={{...S.bigBtn(),marginTop:14}} onClick={()=>setNav(5)}>Create Shoot Plans →</button>
        </>}
      </div>
    );
  }
  ...
  function SectionShoot() {
    return (
      <div>
        {picked.length===0?<div style={{...S.card,color:BRAND.muted}}>Pick some ideas first.</div>:picked.map((idea,i)=>{
          const plan=shoots[idea.title]; const pc=pillarColor(idea.pillar);
          return (
  ...
  function SectionHooks() {
    return (
      <div>
        {picked.length===0?<div style={{...S.card,color:BRAND.muted}}>Pick some ideas first.</div>:picked.map((idea,i)=>{
          const data=hooks[idea.title]; const pc=pillarColor(idea.pillar);
          return (
            <div key={i} style={{...S.card,borderLeft:`4px solid ${pc}`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
                ...
                <button style={S.rainbowBtn} onClick={()=>genHooks(idea)}>{loading==="hooks_"+idea.title?"...":data?"Regen":"Generate Hooks"}</button>
              </div>
              {data&&<><div style={{fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:BRAND.muted,marginBottom:10}}>Hooks</div>{data.hooks?.map((h,j)=><EditableBlock key={j} label={h.type} text={h.text} onSave={(t)=>{const hs=[...data.hooks];hs[j]={...h,text:t};const u={...hooks,[idea.title]:{...data,hooks:hs}};setHooks(u);save({hooks:u});}}/>)}<div style={S.divider}/><div style={{fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:BRAND.muted,marginBottom:10}}>Captions</div>{data.captions?.map((c,j)=><EditableBlock key={j} label={c.platform} text={c.text+(c.cta?"\n\nCTA: "+c.cta:"")} onSave={(t)=>{const cs=[...data.captions];cs[j]={...c,text:t};const u={...hooks,[idea.title]:{...data,captions:cs}};setHooks(u);save({hooks:u});}}/>)}</>}
            </div>
          );
        })}
  ...
  function SectionScheduler() {
    ...
    return (
      <div style={{position:"relative"}}>
        {dragItem&&(
          <div style={{position:"fixed",left:dragPos.x+10,top:dragPos.y+10,zIndex:9999,pointerEvents:"none",padding:"8px 14px",background:pc2(dragItem.item.pillar),color:"#fff",fontSize:12,fontWeight:700,maxWidth:160,boxShadow:"0 8px 24px rgba(0,0,0,0.25)",opacity:0.92}}>{dragItem.item.title}</div>
        )}
        <div style={{...S.card,marginBottom:12}}>
          ...
          <div style={{fontSize:13,color:BRAND.muted}}>Click and drag ideas onto the calendar</div>
        </div>
        <div data-pool="true" style={{...S.card,marginBottom:12}}>
          ...
          {picked.length===0&&<div style={{fontSize:13,color:BRAND.muted}}>Pick some ideas in the Picker step first.</div>}
        ...
        {WEEKS.map(week=>(
          <div key={week} style={{...S.card,marginBottom:10}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:6}}>
              {DAYS.map(day=>{
                const key=`w${week}_${day}`;const slotIdea=slots[key];const isOver=dragOver===key;const pc=slotIdea?pc2(slotIdea.pillar):null;
                return(
                  <div key={day} data-slot={key} style={{minHeight:90,border:`2px ${isOver?"solid":"dashed"} ${isOver?"#4ecdc4":BRAND.border}`,background:isOver?"#e8fffe":slotIdea?pc+"15":"#fafafa",padding:6,boxSizing:"border-box",transition:"border-color 0.1s,background 0.1s"}}>
                    ...
                    {slotIdea?(
                      <div onMouseDown={e=>startDrag(e,slotIdea,key)} onTouchStart={e=>startDrag(e,slotIdea,key)} style={{padding:"6px 7px",background:pc,cursor:"grab",userSelect:"none",WebkitUserSelect:"none",position:"relative"}}>
                        <div style={{fontSize:10,fontWeight:700,color:"#fff",lineHeight:1.3,marginBottom:2,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical"}}>{slotIdea.title}</div>
                        <div style={{fontSize:9,color:"rgba(255,255,255,0.8)"}}>{slotIdea.format}</div>
                        <button onMouseDown={e=>e.stopPropagation()} onClick={e=>{e.stopPropagation();const ns={...slots};delete ns[key];setSlots(ns);save({slots:ns});}} style={{position:"absolute",top:2,right:2,background:"rgba(0,0,0,0.35)",color:"#fff",border:"none",cursor:"pointer",fontSize:10,width:15,height:15,display:"flex",alignItems:"center",justifyContent:"center",padding:0,lineHeight:1}}>×</button>
                      </div>
                    ):(
        ...
        <button style={{...S.smBtn(false),marginTop:4}} onClick={()=>{setSlots({});save({slots:{}});}}>Clear Schedule</button>
      </div>
    );
  }
    ...
    return (
      <div style={S.card}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:26}}>
          {[{l:"Posts Done",v:stats.posted},{l:"Streak",v:stats.streak+"🔥"},{l:"In Pipeline",v:picked.length},{l:"Months",v:monthPlans.filter(Boolean).length}].map(st=>(
            <div key={st.l} style={{padding:18,background:"#f9f9f9",textAlign:"center",border:`1px solid ${BRAND.border}`}}>
              <div style={{fontSize:34,fontWeight:900}}>{st.v}</div>
              <div style={{fontSize:11,color:BRAND.muted,marginTop:4,letterSpacing:0.5,textTransform:"uppercase"}}>{st.l}</div>
            </div>
          ))}
        ...
        <div style={{display:"flex",gap:12,marginTop:22}}>
          <button style={S.bigBtn()} onClick={()=>{const u={...stats,posted:stats.posted+1};setStats(u);save({stats:u});}}>+ Mark Post Done</button>
          <button style={S.outBtn} onClick={()=>{const u={...stats,streak:stats.streak+1};setStats(u);save({stats:u});}}>+ Streak Day</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={S.card}>
        ...
        <div style={{display:"flex",gap:10,marginBottom:14}}>
          <input style={{...S.input,flex:1}} placeholder="Ask your AI advisor..." value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendChat()}/>
          <button style={S.rainbowBtn} onClick={sendChat}>Send</button>
        </div>
        <div style={{display:"flex",gap:10}}>
          <button style={S.outBtn} onClick={()=>{if(chat.length>0){const s=[...savedChats,{date:new Date().toLocaleDateString(),msgs:chat}];setSavedChats(s);setChat([]);save({savedChats:s,chat:[]});}}}>Save & Clear</button>
    ...
    <div style={S.page}>
      <div style={S.topbar}>
        ...
        <div style={{display:"flex",gap:14,alignItems:"center"}}>
          ...
          {isPro?<span style={{fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",...gradText(BRAND.rainbow)}}>✦ CREATOR</span>:<button style={S.smBtn()} onClick={()=>setScreen("upgrade")}>Upgrade</button>}
          <button style={{...S.outBtn,padding:"6px 12px",fontSize:11}} onClick={()=>{if(window.confirm("Reset all data?")){localStorage.removeItem("cs_v2");window.location.reload();}}}>Reset</button>
        </div>
      </div>
